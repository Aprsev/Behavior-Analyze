...
from core.label_compat import as_bool, normalize_polygon, video_matches, video_mask
    ...
    def edit_item(self, item: dict) -> None:
        ...
        if choice:
            ...
            if roi is None or not Path(roi).is_file():
                ...
                return
            command = [sys.executable, str(UNET_ROOT / "edit_keypoint_label.py"),
            ...
            command = [sys.executable, str(UNET_ROOT / "edit_polygon_label.py"),
                       "--video", str(video), "--frame", str(item["frame"]),
